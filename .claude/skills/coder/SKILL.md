...
- **Finish with the right validation** — run `npm run build` (runs `vue-tsc -b && vite build`); fix every compile error before closing. Also run the spec for whatever you touched, and `npx vitest run <spec> --coverage` on critical-service edits. Never report done with a broken build.
