...
- **Cenário** (`ScenarioType`): distribuição do vetor de entrada:
  - `crescente`: [1..n]
  - `decrescente`: [n..1]
  - `aleatorio`: permutação Fisher-Yates determinística com seed (quando `allowDuplicates=true`, amostragem com reposição)
  - `quaseOrdenado`: crescente com ~5% dos pares trocados aleatoriamente
  - `quaseDecrescente`: decrescente com ~5% dos pares trocados aleatoriamente
  - `gaussiano`: valores amostrados de distribuição normal centrada em n/2 (Box-Muller, desvio = n/6); gera duplicatas
  - `organPipe`: formato de tenda — metade crescente, metade decrescente
  - `comOutliers`: crescente com ~1% dos pares deslocados (picos isolados)
...
- `CompareJob`: configuração do benchmark (algoritmos, cenários, tamanhos, replications, seed, timeout, outliers, `allowDuplicates`).
...
- Algoritmo (alto nível):
  - iterar tamanhos × cenários × algoritmos (células);
...
- `generateScenarioArray(size, scenario, seed, allowDuplicates?)` suporta todos os 8 valores de `ScenarioType`. `allowDuplicates` afeta apenas `aleatorio` (amostragem com reposição quando verdadeiro).
...
- `__tests__/services/*`
  - `SeededPrng`: determinismo (`deriveCellSeed`) e geração de cenários (todos os 8 valores de `ScenarioType`) para **fairness**;
